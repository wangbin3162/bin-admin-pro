    ...
    <b-button-group>
      ...
      >
        ...
        <div style="width: 160px">
          ...
          <b-divider
            style="margin: 8px 0"
            v-if="field.headerAttributeFilter && field.valueFilter"
          ></b-divider>
          <!-- Values filter -->
            ...
            <div class="dropdown-list">
              <div v-for="(item, index) in fieldValues" :key="index" style="padding: 2px 10px">
                <b-checkbox v-model="item.checked" @change="handleCheckChange">
                  ...
                  >
                    Missing slot
                    <code>{{ field.valueFilterSlotName }}</code>
                  </slot>
                  <template v-else>
                    {{ item.value }}
                  </template>
...
export default {
  ...
  setup(props) {
    ...
    // 判断是否全选了
    function isCheckAll() {
      const checkCount = props.fieldValues.reduce(
